...
//        Debug.LogError("ResetCenter :  index : " + index + " , localposition : " + closest.localPosition + " , pickingPoint : " + pickingPoint);
...
//             ICoverFlowUnit unit = target.GetComponent<ICoverFlowUnit>();
            Vector3 cp;
            float coverflowoffset = 0.0f;

//            if (unit != null)
//            {
//                cp = unit.GetPoint();
                //coverflowoffset = unit.GetOffSet();
//            }
//            else

            {
                cp = panelTrans.InverseTransformPoint(target.position);
                coverflowoffset = 0.0f;
            }
            ...
            {
                localOffset.x = 0f;
                if (localOffset.y > coverflowoffset)
                    localOffset.y -= coverflowoffset;
                if (localOffset.y < -coverflowoffset)
                    localOffset.y += coverflowoffset;
            }
            ...
            {
                localOffset.y = 0f;
                if (localOffset.x > coverflowoffset)
                    localOffset.x -= coverflowoffset;
                if (localOffset.x < -coverflowoffset)
                    localOffset.x += coverflowoffset;
            }
            localOffset.z = 0f;
...
//                Debug.LogError("panelTrans.localPosition : " + panelTrans.localPosition + " , localOffset :  " + localOffset + " , cp : " + cp + " , cc : " + cc);
