...
{
    public const float FIT_WIDTH = 1080.0f;
    public const float FIT_HEIGHT = 1920.0f;
	...
	protected virtual void Awake ()
    {
        ...
        manualWidth = (int)FIT_WIDTH;
        manualHeight = (int)FIT_HEIGHT;
        fitWidth = true;
        fitHeight = true;
        scalingStyle = Scaling.ConstrainedOnMobiles;
    }
		...
		if (mTrans != null)
		{
			...
			if (calcActiveHeight > 0f)
			{
					...
					!(Mathf.Abs(ls.z - size) <= float.Epsilon))
				{
                    ...
                    float x_value = (NGUITools.screenSize.x / FIT_WIDTH);
                    float temp_value = (FIT_HEIGHT * x_value);
